...
export default function DriverMapPage() {
  ...
  const [pausedWaypointIdx, setPausedWaypointIdx] = useState(null); // 1..n when paused

  const confirmArrival = () => {
    // Yêu cầu: tất cả học sinh tại điểm hiện tại phải có status 'picked_up'
    if (!allStudentsPickedUpAtStop(currentStopIndex)) {
      addAlert("error", "⚠️ Chưa đón đủ học sinh tại điểm này");
      return;
    }
    // resume bus then advance logical stop index
    if (pauseResumeFn) {
      pauseResumeFn();
    }
    ...
    setPausedWaypointIdx(null);
    setPauseResumeFn(null);
              ...
              <BusRoutePause
                ...
                onReachStop={(wpIdx, resumeFn) => {
                  ...
                  if (wpIdx < stops.length) {
                    setPausedWaypointIdx(wpIdx);
                    setPauseResumeFn(() => resumeFn);
